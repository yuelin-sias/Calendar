...
@interface YLCalendarView() <UIScrollViewDelegate>{
    //星期
    UIView * _headView;
    //日历的展示
    UIView * _bodyViewL;
...
-(void)reloadView{
    ...
    _today = [NSDate date];
    _selectModel = [[YLCalendarModel alloc]init];
        ...
        for (int j=0; j<7; j++) {
            ...
            if (_markArray!=nil) {
                for (int i=0; i<_markArray.count; i++) {
                    YLCalendarModel * model = _markArray[i];
...
-(void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    ...
    if ([self.delegate respondsToSelector:@selector(YHBaseCalendarViewScrollEndToDate:)]) {
        YLCalendarModel * model = [[YLCalendarModel alloc]init];
